...
namespace hospital_management.pages
{
    public partial class LoginPage : System.Web.UI.Page
    {
        ...
        private void FetchPatientDetails(string username)
        {
            ...
            {
                string query = "SELECT patientID, name, age, gender, dateOfBirth, contactNumber, email, address, emergencyContact, bloodGroup, allergies, runningMedicines, insurance, profile " +
                               "FROM tbl_Patients WHERE email = @Email";
                ...
                if(reader.Read())
                {
                    ...
                    Session["UserID"] = reader["patientID"].ToString();  // ✅ Now matches what AddAppointment.aspx.cs expects
                    Session["PatientID"] = reader["patientID"].ToString();
                    Session["PatientName"] = reader["name"].ToString();
                    Session["PatientAge"] = reader["age"].ToString();
                    Session["PatientGender"] = reader["gender"].ToString();
                    Session["PatientDOB"] = reader["dateOfBirth"].ToString();
                    Session["PatientContact"] = reader["contactNumber"].ToString();
                    Session["PatientEmail"] = reader["email"].ToString();
                    Session["PatientAddress"] = reader["address"].ToString();
                    Session["PatientEmergencyContact"] = reader["emergencyContact"].ToString();
                    Session["PatientBloodGroup"] = reader["bloodGroup"].ToString();
                    Session["PatientAllergies"] = reader["allergies"].ToString();
                    Session["PatientMedicines"] = reader["runningMedicines"].ToString();
                    Session["PatientInsurance"] = reader["insurance"].ToString();
                    Session["PatientProfilePicture"] = reader["profile"] != DBNull.Value ? reader["profile"].ToString() : "~/Images/default.png";
                }
            }
        }
